<template>
  <div>
    <h1 style="font-family: 'Play', sans-serif;">Current round: {{ CurRound }}</h1>
    <!-- <h1 style="font-family: 'Play', sans-serif;">Time until next check: {{ Tick }}</h1> -->
    <details open>
...
export default {
  ...
  methods: {
      ...
      this.axios.get(`http://${window.location.host}/api/info`)
        .then(result => {
          ...
          if (result.length > 0) {
            ...
            // ? Vertical line of the current round
            // TODO Rewrite the hack
            // I DON'T KNOW WHY THE FOLLOWING CODE DOESN'T WORK FOR YOU)
            /*
              markLine: {
                silent: true,
                symbol: [],
      ...
      for (var Info of infos) {
        ...
        for (var teamSeries of this.teamsSeries) {
          if (teamSeries['name'] === teamName) {
            ...
            this.maxScore = this.maxScore < growedData * 1.1 ? growedData * 1.1 : this.maxScore
          } else if (teamSeries['name'] === 'Current round') {
    ...
    tickFunction () {
      console.log("tickFunction called")
      if (this.time > 0) {
        this.time -= 1
      }
    }
