...
def return_status_ascis(status_code):
    if status_code == "status-110" or status_code == "status_" or 'rgba(248, 222, 34' in status_code:
        return "CHECK FAILED"
    elif status_code == 'status-101' or status_code == "status_up" or 'rgba(168, 223, 142' in status_code:
        return "UP"
    elif status_code == 'status-102' or status_code == "status_corrupt" or 'rgba(117, 194, 246' in status_code:
        return "CORRUPT"
    elif status_code == 'status-103' or status_code == "status_mumble" or 'rgba(217, 137, 181' in status_code:
        return "MUMBLE"
    elif status_code == 'status-104' or status_code == "status_down" or 'rgba(252, 41, 71' in status_code:
        return "DOWN"
    else:
        return "ERROR, UNKNOWN STATUS CODE"
...
def get_teams_info(driver, soup, scoreboard_type):
    if driver:
        if scoreboard_type == "ASCIS":
            teams_info = []
            while not teams_info:
                teams_info = driver.find_elements_by_class_name("row")[1:]
            teams = []
            for team in teams_info:
                place_element = team.find_element_by_class_name("number")
                try:
                    img_element = place_element.find_element_by_tag_name('img')
                    img_src = img_element.get_attribute('src')
                    if 'gold-medal' in img_src:
                        place = 1
                    elif 'silver-medal' in img_src:
                        place = 2
                    elif 'bronze-medal' in img_src:
                        place = 3
                except:
                    place = int(place_element.find_element_by_tag_name('span').text.strip())

                team_data = team.text.split("\n")
                if not isinstance(team_data[0], int):
                    try:
                        team_data[0] = int(team_data[0])
                    except ValueError:
                        team_data.insert(0, place)
                else:
                    pass
                teams.append({
                    'name': team_data[1].strip(),
                    'place': place,
                    'score': float(team_data[3].strip()),
                    'ip': team_data[2].strip(),
                    'services': get_services_info_forcad2(team, driver, team_data)
                })
            return teams
        else:
            teams_info = []
...
def get_services_info_forcad2(team, driver, team_data):
    services = team_data[4:]
    statuses = get_status_info_ascis(driver, team)
    services_name = get_services(driver, None)
    services_info = [{
        "name": services_name[i % len(services)],
        "sla": float(services[i * 3].split(':')[1].strip()[:-1]),
...
def get_status_info_ascis(driver, team):
    services_name = get_services(driver, None)
    services = team.find_elements_by_class_name("service-cell")
    return {services_name[number % len(services)]: return_status_ascis(ele.find_element_by_class_name("status-circle").value_of_css_property('background-color')) for number, ele in enumerate(services)}
