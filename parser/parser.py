...
SCOREBOARD = os.getenv('SCOREBOARD', 'https://final-ad.ascis.vn/')
HEADLESS = True if os.getenv('TYPE', 'hackerdom') in ["forcad", "ASCIS"] else False
TEAM = os.getenv('TEAM', '[VN1][SOICT] SSH1141')
...
# Getting information by command name or IP address
if TEAM_IP:
    AD.get_info_by_ip(TEAM_IP)
elif TEAM_NAME:
    AD.get_info_by_name(TEAM_NAME)

# If there is no information about the current round
